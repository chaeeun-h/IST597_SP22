# -*- coding: utf-8 -*-
"""Assignment2_solution_batch_normalization

Automatically generated by Colaboratory.

Original file is located at
    https://colab.research.google.com/drive/1PvrQeA8fW3fACDP8q_MwNwolpFcQJbBq
...
class MLP(object):
  ...
  def compute_output(self, X):
    ...
    bhat0 = tf.nn.batch_normalization(hhat0, tf.reduce_mean(hhat0), 
                                     tf.math.reduce_variance(hhat0), None, None, 1e-12)

    # Compute values in hidden layer 1
    what1 = tf.matmul(bhat0, self.W1) + self.b1
    hhat1 = tf.nn.relu(what1)
    bhat1 = tf.nn.batch_normalization(hhat1, tf.reduce_mean(hhat1), 
                                      tf.math.reduce_variance(hhat1), None, None, 1e-12)

    # Compute values in hidden layer 2
    what2 = tf.matmul(hhat1, self.W2) + self.b2
    hhat2 = tf.nn.relu(what2)
    bhat2 = tf.nn.batch_normalization(hhat2, tf.reduce_mean(hhat2),
                                      tf.math.reduce_variance(hhat2), None, None, 1e-12)
...
for epoch in range(NUM_EPOCHS):
  ...
  acc = 0
  train_ds = tf.data.Dataset.from_tensor_slices((X_train, y_train)).shuffle(25, seed=epoch*(9630)).batch(20)
...
for i in range(10):
  ...
  test_err = 0
  test_var = 0
  test_ds = tf.data.Dataset.from_tensor_slices((X_test, y_test)).shuffle(25).batch(20)
  for inputs, outputs in test_ds:
    preds = mlp_on_gpu.forward(inputs)
    test_loss_total = test_loss_total + mlp_on_gpu.loss(preds, outputs)
    mlp_on_gpu.backward(inputs, outputs)
...
plt.plot(accuracy_fashion, label = 'accuracy')
plt.plot(err_fashion, label='standard error')
plt.plot(var_fashion, label='variance')
plt.legend(loc='upper right', bbox_to_anchor = (1,1.35))
plt.xlabel('Trial #')
# plt.ylabel('Count')
plt.title('Batch Normalization - Fashion MNIST')
plt.show()
...
# plt.plot(var_mnist, label='variance')
plt.legend(loc='upper right', bbox_to_anchor = (1,1.35))
plt.xlabel('Trial #')
